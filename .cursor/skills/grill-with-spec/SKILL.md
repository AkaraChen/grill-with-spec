...
description: Relentless design interview that produces a single SPEC.md, an RFC 2119-style, language-agnostic system specification written and sharpened inline as decisions settle. Use when the user wants to grill a design into a spec, write or revise a SPEC.md, or asks to "spec this out" before implementation.
...
The deliverable is `SPEC.md`, not the conversation. A reader who has never seen the chat, human or
coding agent, must be able to implement the system from the file alone.
...
4. Gather facts before asking anything: existing code, README, `CONTEXT.md` glossary, ADRs, and the
...
- A question whose answer depends on another question still open in this round belongs to a later
  round.
- When a frontier question needs a fact from the environment, dispatch a sub-agent or look it up.
